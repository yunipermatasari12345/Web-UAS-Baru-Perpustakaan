                            ...
                            <div class="col-md-4">
                                <div class="mb-3">
                                    ...
                                    <div class="border rounded p-3 text-center" id="imagePreviewContainer" style="min-height: 200px; background-color: #f8f9fa;">
                                        <img id="imagePreview" src="{{ $book->cover_image_url }}" alt="Preview" style="max-width: 100%; max-height: 180px; {{ $book->cover_image ? '' : 'display: none;' }}">
                                        <div id="imagePlaceholder" style="{{ $book->cover_image ? 'display: none;' : '' }}">
                                    ...
                                    <div class="form-text">Format: JPG, PNG, GIF. Maksimal 2MB. Kosongkan jika tidak ingin mengubah cover.</div>
                                    @error('cover_image')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <div class="mb-3">
                                    ...
                                    <input type="number" class="form-control @error('year_published') is-invalid @enderror" id="year_published" name="year_published" value="{{ old('year_published', $book->year_published) }}" min="1900" max="{{ date('Y') + 1 }}" required>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-control @error('yuni_category_id') is-invalid @enderror" id="yuni_category_id" name="yuni_category_id" required>
                                        ...
                                        @foreach($categories as $category)
                                            <option value="{{ $category->id }}" {{ old('yuni_category_id', $book->yuni_category_id) == $category->id ? 'selected' : '' }}>
                                                {{ $category->name }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control @error('yuni_publisher_id') is-invalid @enderror" id="yuni_publisher_id" name="yuni_publisher_id" required>
                                ...
                                @foreach($publishers as $publisher)
                                    <option value="{{ $publisher->id }}" {{ old('yuni_publisher_id', $book->yuni_publisher_id) == $publisher->id ? 'selected' : '' }}>
                                        {{ $publisher->name }}
                                    </option>
                                @endforeach
                            </select>
...
function previewImage(input) {
    ...
    if (input.files && input.files[0]) {
        ...
        // If no new file selected, show current image or placeholder
        const currentSrc = preview.src;
        if (currentSrc && !currentSrc.includes('default-cover.jpg')) {
            preview.style.display = 'block';
            placeholder.style.display = 'none';
            container.style.backgroundColor = 'white';
        } else {
